...
s3_ego_mc.py
============
CARLA 0.9.16 physics simulation — Motorcycle lateral departure scenario.

Author:  Tejas Manjunath
Project: Lateral Intrusion ADAS Gap — AEB Blind Zone Study

What this script does:
  Simulates a motorcycle (Kawasaki Ninja) departing laterally from an
...
Outputs (saved to s3_ego_mc_output/):
...
SPEED_KMH = 80.0  # Highway cruise speed for both ego and motorcycle
...
# Scenario timing
STABLE_T = 5.0  # Stable phase duration before motorcycle begins lateral drift
RUN_DURATION = 16.0  # Total simulation runtime in seconds

# Lateral intrusion parameters
LAT_SPEED = 0.7  # Motorcycle lateral drift speed in m/s (Euro NCAP motorcycle leaving-carriageway scenario)
...
def main():
    ...
    Phases:
      ...
      2. Departure phase (5s-9s): Motorcycle drifts laterally at 0.7 m/s
    ...
    try:
        ...
        # Get adjacent lane waypoint for motorcycle
        ...
        # Calculate side camera yaw to point toward motorcycle lane
        ...
        # Side camera: points toward motorcycle lane
        ...
        while True:
            ...
            if not physics_on:
                ...
                # Update motorcycle position (with lateral drift after departure)
                if not departed:
            ...
            # Trigger motorcycle departure at stable phase end
            ...
            if t - last_print >= 1.0:
                ...
                print(f"t={t:5.1f}s {ph}  EGO({ep.x:.0f},{ep.y:.1f})  "
                      f"MC({mp.x:.0f},{mp.y:.1f})  lat={lat:.2f}m  side={sh} TTC={ttc:.2f}s  AEB={aeb_triggered}")
                last_print = t
        ...
        # Reconstruct per-tick position log from ego_path and mc lateral drift
        metrics_log = []
